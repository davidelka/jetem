...
- **Prefix chords** — core: `r` recall, `a` literal Ctrl-A. `mux.py`: `|`/`v` `-`/`s` split, `h`/`j`/`k`/`l` (or arrows) focus, `x` close. `ai.py`: `i` explain, `c` suggest, `m` model. `richout.py`: `t` render table/tree. `theme.py`: `y` cycle theme, `p` flip terminal background.
